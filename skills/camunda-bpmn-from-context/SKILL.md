...
- **Anchor mapping rule.** Every BPMN element `id` MUST map to a canonical `A61.AnchorID` in the manifest. The `element_kind` field uses BPMN 2.0 XML tag names (camelCase) covering the subset documented in `references/support-matrix.md`: events (`startEvent`, `endEvent`, `intermediate{Catch,Throw}Event`, `boundaryEvent`), tasks (`task`, `userTask`, `serviceTask`, `receiveTask`, `sendTask`, `scriptTask`, `businessRuleTask`, `manualTask`), subprocess-like (`callActivity`, `subProcess`, `transaction`, `adHocSubProcess`), gateways (`exclusiveGateway`, `parallelGateway`, `inclusiveGateway`, `eventBasedGateway`, `complexGateway`), flows/collaboration (`sequenceFlow`, `messageFlow`, `participant`, `lane`, `laneSet`), and data/annotations (`dataObject`, `dataObjectReference`, `dataStoreReference`, `textAnnotation`, `association`, `group`). Event definitions are attributes of event elements, not element kinds themselves. Unmapped flow elements are treated as fabrications.
