...
| v1.3.5 | 1.3.2 | `a0cbc336` | Dashboard polish — Mermaid traceability graph + bpmn-js inline viewer + full-text search (nav search box + JSON index) + Chart.js KPI trends + keyboard shortcuts (`/` focus search, `g <letter>` nav). Bundled 4 vendor libs locally under `scripts/dashboard/static/vendor/` (~4.4MB committed): Mermaid 10.9.1, bpmn-js 17.9.1 NavigatedViewer + 2 stylesheets, Chart.js 4.4.0. Per locked v1.3.3 design (no CDN telemetry leak). Templates: traceability.html now renders Mermaid flowchart ABOVE the existing nested table (JS-disabled fallback); sidecar_view.html embeds bpmn-js viewer (BPMN-only branch) with collapsible XML source; phase7_index.html renders Chart.js line chart per numeric KPI extracted from telemetry runs. Search index emits at dashboard root; safe-ID filter from v1.3.3 R1 lesson #15 reapplied so unsafe proposal IDs don't appear as dead links. Vendor subdir copy fixed (pre-fix `iterdir()` was non-recursive — silently skipped vendor/). 19 new tests (60→79). Operator-tooling only — script not skill, canon-neutral, manifest stays at 1.3.2. |
| v1.3.4 | 1.3.2 | `a0cbc336` | Knowledge consolidation — operator runbook + cookbook + CONTRIBUTING. Pure docs; canon-neutral; manifest stays at 1.3.2. Adds `docs/dashboard_runbook.md` (operator how-to for v1.3.3 dashboard with 5 named workflows + page map + failure modes), `docs/CONTRIBUTING.md` (15 self-review lessons distilled + Codex review workflow + 5 hard rules + canon-bump 8-step + per-release-shape round expectations), 4 cookbook recipes under `docs/cookbook/` (add_contract_exporter / add_audit / add_sidecar / add_phase7_tunable — each with file scaffold + decision matrix + common pitfalls). Updates `docs/architecture_overview.md` (skill count 23→32 + added Phase 3 dev-handoff + Stage 6 exporters + dbml sidecar + INV-08/09/10 + dashboard + Phase 7 + audits paragraphs; corrected later-section stale numbers per Codex R1) and `.claude-plugin/plugin.json` description (29→32 + added Stage 6 + dashboard mentions). Codex review: 3 rounds (R1 3 MAJOR + 2 MINOR all from inventing instead of reading real code; R2 1 MAJOR + 2 MINOR post-fix drift; R3 APPROVE). 16 self-review lessons accumulated (lesson #16 NEW: read existing code before writing recipes about it). 2493 tests unchanged. |
| v1.3.3 | 1.3.2 | `a0cbc336` | Static-HTML operator dashboard — first operator-facing UI for the BSA plugin family. Read-only viewer over canonical artifacts + handoff packets + audit reports + sidecar diagrams + Phase 7 telemetry. New `scripts/generate_dashboard.py` (~530 LOC, stdlib + jinja2 + markdown-it-py) + `scripts/dashboard/{loaders,renderers}.py` + 15 Jinja2 templates + 3 vanilla JS files (filterable table, claim filter, clipboard) + `style.css` (~600 lines, light/dark mode auto). Generates 20+ static HTML pages under `<workspace>/analysis/handoff/dashboard/`. Operator opens `dashboard/index.html` in any browser — no HTTP server, no backend, no canonical writes. CLI flags: `--workspace`, `--output-dir`, `--filter` (comma-separated page subset), `--watch` (polling, 2s interval, no watchdog dep; browser auto-refreshes via injected meta-refresh tag), `--print-only`, `--quiet`. Pages: overview index → 10 sortable+filterable A-table pages with anchor-ID cross-links → claim-layer view → traceability matrix → audit dashboards (MD→HTML) → handoff packets H1-H4 → contract exports (OpenAPI/AsyncAPI/proto with anchor manifest mapping) → sidecar diagrams (C4/BPMN/DBML as code blocks + anchor manifest, lazy: operator runs proper viewer separately) → Phase 7 L2 proposals (summary + inline-styled unified-diff viewer + clipboard `git apply` command, shlex-quoted per lesson #4). 47 new tests (47/47 first-run pass; 14 v1.2.19-v1.3.2 lessons baked in pre-Codex including shlex.quote() for operator-pasteable commands). **Operator-tooling only — script not skill, canon-neutral, manifest stays at 1.3.2**. NEVER runs git/commit/push/edit-canonical (mirrors v1.2.19 patcher's never-list). |
| v1.3.2 | 1.3.2 | `a0cbc336` | Stage 6 proto exporter — third and final contract exporter in the v1.3.x line; completes the OpenAPI / AsyncAPI / proto trio. New `skills/proto-from-context/` (SKILL.md + integration-contract.md + anchor_manifest.schema.json — all 3 in POLICY_GLOBS, mirroring v1.3.0 + v1.3.1) + `scripts/generate_proto.py` (stdlib-only — proto syntax is text, no pyyaml dep; reads canonical A61, materializes path-shaped contract-class anchors as proto3 Service + Rpc pairs with Service deduplication; emits `analysis/handoff/contracts/proto/services.proto` + `anchor_manifest.json`). Path-shape regex identical to v1.3.0/v1.3.1 post-R2 form `^/[A-Za-z0-9_/{}.~-]*$` — atomic regex-level rejection of `%` closes recursive-decode attack surface. Brace-balance scan reused from v1.3.0/v1.3.1 R2 BUT with **tightened parameter-name charset** for v1.3.2 — uses the proto3 field-name grammar `^[A-Za-z_][A-Za-z0-9_]*$` (NOT v1.3.1 AsyncAPI's permissive `^[A-Za-z0-9_-]+$`). Hyphens (`{order-id}`) and leading digits (`{9id}`) are rejected at gate time as `element_id_not_path_shaped` because they would render as illegal proto3 field names — v1.3.2 R1 lesson #14 (when porting a charset constant across exporters in a family, do NOT reuse verbatim — re-derive from the new exporter's downstream consumer grammar). **NEW for v1.3.2**: proto-identifier synthesis pass (`_synthesize_service_method`) — splits ElementID, filters template segments, PascalCases first non-template segment to `service` name and remaining segments to `method` name (defaults `Invoke` when only one literal segment), validates both against proto3 identifier regex `^[A-Za-z_][A-Za-z0-9_]*$`. Path templates (`{order_id}`) become `string <name> = <N>;` fields in the generated request message (sequential tags from 1). All emitted rpcs default to **unary**; operator changes to streaming (`stream Req`/`stream Resp`) post-export. NEW `--package` CLI flag with proto3 package validation `^[A-Za-z_][A-Za-z0-9_]*(\.[A-Za-z_][A-Za-z0-9_]*)*$` (default `bsa.contracts`). **Skeleton-only**: operator enriches Request/Response messages with real proto3 fields + chooses streaming + adds well-known-type imports after export. Dot-style ElementIDs (`OrderService.CreateOrder`) deferred to v1.3.2.x. NEVER runs git/commit/push/protoc/edit-canonical (mirrors v1.2.19 patcher's never-list). 97 new tests (97 first-run pass; 13 v1.2.19-v1.3.1 domain-gotcha lessons baked in pre-Codex including #12 spec-required-fields-per-element-kind read for proto3 + #13 reach-equality-inside-template applied to synthesis-output charset). 3 new POLICY_GLOBS entries → canon hash + manifest both bump in lockstep. **Completes the v1.3.x exporter family**. |
| v1.3.1 | 1.3.1 | `c7442dd1` | Stage 6 AsyncAPI exporter — second of three contract exporters in the v1.3.x line. New `skills/asyncapi-from-context/` (SKILL.md + integration-contract.md + anchor_manifest.schema.json — all 3 in POLICY_GLOBS, mirroring v1.2.11 dbml + v1.3.0 openapi pattern) + `scripts/generate_asyncapi.py` (stdlib + pyyaml; reads canonical A61, materializes path-shaped contract-class anchors as AsyncAPI 3.0 Channel + send-Operation pairs; emits `analysis/handoff/contracts/asyncapi/asyncapi.yaml` + `anchor_manifest.json`). Channel-shape regex identical to v1.3.0 OpenAPI exporter post-R2 form `^/[A-Za-z0-9_/{}.~-]*$` — atomic regex-level rejection of `%` closes recursive-decode attack surface. **Channel Address Expressions** (`{order_id}`) emit `channels[*].parameters` per AsyncAPI 3.0 spec; parameter names match `^[A-Za-z0-9_-]+$` (hyphens allowed; dots / spaces / slashes rejected at the shape gate). **Skeleton-only**: operator enriches channels with real message payloads + per-operation `send`/`receive` direction + parameter `schema`/`location` after export. Dot-style channel addresses (`user.signed_up`) deferred to v1.3.1.x — v1.3.1 supports only path-style for symmetry with v1.3.0 OpenAPI exporter. NEVER runs git/commit/push/edit-canonical (mirrors v1.2.19 patcher's never-list). 68 new tests (61 first-run + 7 R1+R2 fix regressions; 13 v1.2.19-v1.3.0 domain-gotcha lessons baked in pre-Codex). 3 new POLICY_GLOBS entries → canon hash + manifest both bump in lockstep. |
| v1.3.0 | 1.3.0 | `c5f7f87b` | Stage 6 OpenAPI exporter — first of three contract exporters in the v1.3.x line. New `skills/openapi-from-context/` (SKILL.md + integration-contract.md + anchor_manifest.schema.json — all 3 in POLICY_GLOBS, mirroring the v1.2.11 dbml-from-context registration pattern) + `scripts/generate_openapi.py` (stdlib + pyyaml; reads canonical A61, materializes path-shaped contract-class anchors as OpenAPI 3.1 placeholders, emits `analysis/handoff/contracts/openapi/api.yaml` + `anchor_manifest.json`). **Skeleton-only**: operator enriches paths with real schemas after export; markdown-driven path discovery deferred. NEVER runs git/commit/push/edit-canonical (mirrors v1.2.19 patcher's never-list). Path-shape regex (final R2 form) `^/[A-Za-z0-9_/{}.~-]*$` — ASCII-exact, no `%` (atomic regex-level rejection of percent-encoding closes the recursive-decode attack surface; encoded traversal at any depth surfaces as `element_id_not_path_shaped`). Plus literal `..` traversal check (`element_id_path_traversal` reason) and balanced/non-empty/non-nested/well-ordered `{...}` template scan (lessons from v1.2.16 R4 + v1.2.19 R1-FIX-1). 48 new tests (48/48 first-run pass — self-review checklist + 6 v1.2.19 domain-gotcha checks applied). 3 new POLICY_GLOBS entries → canon hash + manifest both bump in lockstep. **First canon-bumping release since v1.2.17**. Establishes the v1.3.x exporter line. |
