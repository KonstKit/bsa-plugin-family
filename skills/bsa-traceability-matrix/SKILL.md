...
- **Foreign-key integrity** — every A72 row's StoryID, ClaimID, SourceID MUST resolve to an existing row in A70, A59, A50 respectively. Applies to ALL rows including `a51-routed` (deferral is about acceptance, not ID resolution). Documented in `x-bsa-foreign-key-rules.applies_to_all_rows=true`; **executable at the F5 hook layer as of v1.1.3 via `write_validator._apply_foreign_key_rules` + `_SiblingArtifactCache`** (closes `[TODO-S8-02-X-ARTIFACT-FK]`). Pre-v1.1.3 the rule was skill-self-validated only.
- **Claim-source consistency** — for ALL rows (including `a51-routed`), `A59[ClaimID].SourceID` MUST equal this row's SourceID. The matrix is a JOIN; an internally inconsistent join defeats traceability. There is no LinkType exemption for this rule. Same enforcement vehicle as foreign-key integrity above (via `claim_source_consistency: true` in `x-bsa-foreign-key-rules`).
...
- ~~`[TODO-S8-02-X-ARTIFACT-FK]`~~ — **CLOSED in v1.1.3.** Hook-layer enforcement landed via `governance/schemas/write_validator.py::_apply_foreign_key_rules` + `_SiblingArtifactCache`. Any A72 write now triggers per-row sibling lookups (StoryID in A70, ClaimID in A59, SourceID in A50) and `claim_source_consistency` check (this row's ClaimID's A59 SourceID must equal this row's SourceID). Pre-v1.1.3 the rule was skill-self-validated only. Same vehicle as `[TODO-S8-01-X-ARTIFACT-NFR-COVERAGE]` for A71.
- ~~`[TODO-S8-02-INCREMENTAL-MATRIX]`~~ — **CLOSED in v1.2.2.** Operator-side helper at `scripts/a72_incremental_diff.py` computes per-row hashes of A70/A59/A50/A62 inputs and compares to a stored cache at `analysis/canonical/.a72_incremental_state.json` (operator-side, NOT canonical state, NOT F5-validated, NOT in POLICY_GLOBS — safe to delete; deletion forces a full rebuild). Output classifies each upstream row as `added` / `modified` / `removed` / `unchanged`. The skill scopes its A72 re-emission accordingly: for `unchanged` rows that already have A72 entries, copy them forward; for added/modified/removed, recompute the affected triples. CLI flags: `--force-rebuild` (signals full rebuild via exit 1), `--update-cache` (refresh after run), `--json` (machine-readable for downstream tooling). Stdlib-only; cache file uses atomic `mktemp` + `os.replace` (same-FS guarantee).
- ~~`[TODO-S8-02-LINK-STRENGTH-OVERRIDE]`~~ — **CLOSED in v1.2.1.** A51 IssueType enum extended with `link_strength_override` (governance/schemas/a51.schema.json + shared-control-surface-contracts.md updated in lockstep). Operator workflow: when an A72 row's LinkStrength deviates from the default formula (T1/T2 → high; T3 → medium; T4/T5 → low), populate `A51Ref` with an A51 row of `IssueType=link_strength_override`. The A51 row carries the rationale (e.g., "low-tier source corroborated by independent observation not bound at intake"; "high-tier source applies only partially to this story scope"). Recommended `BlockingStatus=informational` (override is annotation, not a gate). Distinct from generic `decision_needed` because downstream KPI tooling can filter overrides separately to detect fixture-shape drift.
