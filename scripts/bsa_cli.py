...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p_mat.add_argument(
        ...
        help=(
            ...
            "v1.4.15 audit P2 narrowed claim: "
            "the cap bounds (i) the rendered BODY content of each "
            "staged source, (ii) ACCUMULATION across body_parts / "
            "chunks / inline_media lists during pptx slide-walk and "
            "email part-walk, AND (iii) `_HTMLToMarkdown` rendered "
            "output during streaming parse. Cap-marker emission is "
            "reliable across these paths, so the post-conversion "
            "safety net intentionally PRESERVES staged output that "
            "exceeds the cap when the marker is present — keeping "
            "structural sections (Attachments, headers) intact. "
            "Net file size CAN therefore exceed the cap by the "
            "size of those sections plus the canonical footer; "
            "the cap is a body-rendering bound, not a hard file-"
            "size ceiling. "
            "REMAINING BOUNDARY (cap does NOT bound): "
            "(a) the underlying library decode that materializes a "
            "document once before our extractor sees it — pypdf, "
            "python-docx, python-pptx (zip→OOXML), stdlib email, "
            "extract-msg, and the html standalone path "
            "(`path.read_bytes()` + full charset decode); "
            "(b) ONE-AT-A-TIME element materialization inside python-"
            "pptx (a single huge `run.text`, `cell.text`, or notes "
            "paragraph still allocates the full element string before "
            "we slice — bounded by element size, which is bounded by "
            "the source deck file size, not the cap); "
            "(c) OCR `pytesseract.image_to_string` per-page output "
            "before it's appended to the page-collected buffer. "
            "Use --max-mb to bound source bytes if you need an "
            "earlier guard against zip-bomb-style compressed inputs."
        ),
    )
    p_mat.add_argument(
        "--ocr-timeout-sec",
