...
1. **Two T4 sources producing contradictory attestations**: under the tier-delta ≤ 1 rule from `reliability_tier_spec.md`, same-tier conflicts route to `A51`, `BlockingStatus=hard`. The fixture `A51` carries that row (`A51-002`) with `IssueType=contradiction` — authored under the pre-v1.1.1 enum (when `cross_tier_contradiction` was a doc-only concept and `contradiction` was the canonical A51 value). v1.1.1 promoted `cross_tier_contradiction` to a first-class A51 IssueType enum value, so newly-authored fixtures should prefer `IssueType=cross_tier_contradiction` for the tier-delta ≤ 1 contested case; this fixture intentionally retains `IssueType=contradiction` to preserve the v1.0.x regression baseline (both values are valid against the v1.1.1 schema enum).
