...
Current release: **`bsa-full@1.2.11` manifest** with `v1.2.11` git tag. v1.2.x line progression: v1.2.0 (first canon bump since v1.1.6 — one-line cross-ref), v1.2.1 (A51 `link_strength_override` enum — closes TODO-S8-02-LINK-STRENGTH-OVERRIDE), v1.2.2 (A72 incremental-diff helper — closes TODO-S8-02-INCREMENTAL-MATRIX), v1.2.3 (A71 runnable test export — closes TODO-S8-01-RUNNABLE-EXPORT; emits Cucumber `.feature` + pytest-bdd + jest-cucumber artifacts via `scripts/a71_runnable_export.py`), v1.2.4 (Phase 7 telemetry foundation L1a — KPI-001 + KPI-006 capture via `scripts/phase_7_telemetry_collector.py` + `governance/schemas/telemetry_run.schema.json`; canon-neutral, manifest stayed at 1.2.3), v1.2.5 (A70 negative-path scenario suggestions — closes TODO-S8-01-NEGATIVE-PATH-HEURISTICS; scans A70 AcceptanceCriteria for temporal/boundary/comparison measurable language and emits boundary + off-by-one suggestions via `scripts/suggest_negative_path_scenarios.py`), v1.2.6 (end-to-end sidecar fixture `project_0004_sidecar_e2e/` with 16 cross-reference tests — canon-neutral), v1.2.7 (A61 schema formalization — `governance/schemas/a61.schema.json` + row-shape F5 enforcement, canon-neutral), v1.2.8 (A61 cross-row F5 enforcement — executable `x-bsa-anchor-binding-rules` extension adds SourceClaimID FK to A59 + AnchorID uniqueness, canon-neutral), v1.2.9 (C4 relationship `view_element_id` convention — `rel_<from>_<connector>_<to>` derivation documented in the C4 sidecar integration contract; closes the v1.2.6 round-1 scope-down; canon-bumping via integration-contract edit), v1.2.10 (generic `x-bsa-uniqueness-rules` extension — extracts A61's uniqueness logic into a schema-agnostic executable extension; canon-neutral), v1.2.11 (third BSA sidecar `dbml-from-context` — relational-schema text with full e2e fixture coverage; closes half of the pre-v1.2.11 open follow-up at `docs/sidecar_inventory.md`; canon-bumping via SKILL.md + integration-contract + anchor-manifest schema additions). The v1.1.x line ran from v1.1.0 (Phase 3 close) through v1.1.19 (anonymization regression test), with the manifest stable at 1.1.6 from v1.1.6 onwards (v1.1.7..v1.1.19 were all canon-neutral operator-tooling releases — anonymization, docs, CI/CD, security audit, perf bench, Phase 7 foundation, sidecar polish, strict-on-hard-A51 mode, shell import drivers, sidecar registry, anonymization regression test). v1.2.0 opens the v1.2.x line with a one-line cross-ref edit in `governance/immutable_invariants.md` → `docs/phase_7_design.md` (the long-deferred reverse cross-ref to the v1.1.14 Phase 7 foundation). Phase 0-2 (MVP) closed at v1.0.0 (Sprint 4.5); Phase 3 (dev-handoff) closed at v1.1.0 (Sprint 9).
