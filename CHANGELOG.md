...
- `.github/workflows/ci.yml` — CI pipeline on push/PR to main. Two jobs: `project-validators` (Python 3.11 + 3.12 matrix) runs structural lint, inventory audit (with idempotency check), privacy scan (with idempotency check), and pytest unit suite; `sidecar-validators` runs c4-plantuml validator + self-tests, BPMN XML lint via xmllint, and INoT smoke evals (non-blocking in Phase 0). Concurrency guarded per-ref to avoid duplicate spend on force-pushes. Timeout 5 min per job (US-S0-03).
