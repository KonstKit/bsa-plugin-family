...
- ~~`[TODO-S9-LIVE-API]`~~ — **CLOSED in v1.1.6.** `scripts/backlog_live_apply.py` ships with three platform clients (Jira REST, Linear GraphQL, GitHub REST). Idempotent per-row (`bsa-{StoryID}-{canon_hash_prefix}` key); dry-run by default; exponential backoff on 429/5xx with retry-budget cap (5); per-row deadline (30s); partial-failure tolerant (continue + non-zero exit). Tokens flow ONLY via env-var indirection (`--*-token-env=BSA_*_TOKEN`); NEVER appear in CLI args, JSONL log records, or the F5-validated `live_api_response.json`. Auth-header values scrubbed in error messages. Per-attempt JSONL log under `analysis/handoff/live_api_log.jsonl` (NO bodies, NO headers). Idempotency state in `analysis/handoff/live_api_response.json` (new F5 dispatcher entry + `live_api_response.schema.json`). Operator-side import drivers (`scripts/jira_import_from_export.sh` etc.) remain a v1.2+ candidate for operators who prefer shell over Python.
