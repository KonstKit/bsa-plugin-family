...
def _default_scan_paths(repo_root: Path) -> list[Path]:
    ...
    v1.4.11 hotfix (closes coverage gap reported on v1.4.8):
    earlier `_default_scan_paths()` only recursed `skills/`, missing
    `commands/*.md` (7 files) AND nested `docs/<sub>/*.md` (19 files
    in docs/cookbook/, docs/retros/) AND any future nested
    `governance/<sub>/*.md`. CI gate was false-clean for those
    paths. Fix: recurse `commands/`, `docs/`, `governance/` the
    same way `skills/` is recursed (with the same vendored-path
    exclusion list)."""
    ...
    paths: list[Path] = []
    # Recursive scan dirs: skills/ + commands/ + docs/ + governance/.
    # All four hold operator-facing markdown that can carry stale
    # cross-refs (skill→file, command→skill, doc→source, etc.).
    for top in ("skills", "commands", "docs", "governance"):
        top_dir = repo_root / top
        if top_dir.is_dir():
            for p in sorted(top_dir.rglob("*.md")):
                if _accept(p):
                    paths.append(p)
