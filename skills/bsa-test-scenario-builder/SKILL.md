...
- ~~`[TODO-S8-01-NEGATIVE-PATH-HEURISTICS]`~~ — **CLOSED in v1.2.5.** Operator-side helper at `scripts/suggest_negative_path_scenarios.py` (stdlib-only) scans every A70 `AcceptanceCriteria` cell for three measurable-language pattern classes and emits per-story boundary + negative-path scenario suggestions: **TEMPORAL** (`within N seconds|minutes|hours|days|business days|business hours|ms|s|m|h|d` → suggest exactly-at-SLA + just-past-SLA scenarios), **BOUNDARY** (`at least N`, `at most N`, `no more than N`, `no fewer than N`, `exactly N` → suggest at-boundary + off-by-one scenarios; `exactly N` emits BOTH N-1 and N+1 negative paths), **COMPARISON** (`more than M`, `less than M`, `greater than M`, `fewer than M`, `larger than M`, `smaller than M`, `>= M`, `<= M`, `> M`, `< M`, `≥ M`, `≤ M` → suggest at-boundary + ±1 negative path on the failing side). Pattern classes are evaluated most-specific first (TEMPORAL → BOUNDARY → COMPARISON) and a covered-range overlap check prevents `no more than N` from ALSO emitting a `more than N` COMPARISON match (the `seen_spans` exact-equality guard alone would not catch this — overlap is range-based). Output lands at `analysis/handoff/negative_path_suggestions.md` (NOT canonical; safe to delete + regenerate). The helper deliberately does NOT auto-emit A71 rows — A71 row authoring needs human judgment about what's actually testable in the system under test. Same fail-CLOSED CSV parsing as v1.2.2/v1.2.3 (header validation; missing required cells fail the run). CLI: `--workspace`, `--output-path`, `--json` (machine-readable JSON for downstream tooling), `--print-only`, `--quiet`.
