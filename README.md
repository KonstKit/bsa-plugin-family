...
**32 skills** (9 main-cycle workers + 5 discovery workers + 5 auditors + 5 Phase-3 workers + 3 sidecars `c4-plantuml` / `dbml` / `camunda-bpmn` / contract exporters: `openapi`/`asyncapi`/`proto` + 1 orchestrator + 1 meta) • **7 slash-commands** + 1 Phase-3 composite (`/bsa-dev-handoff`) • **3 safety hooks** • **9 golden fixtures** (4 happy-path + 5 adversarial) • **2826 unit tests** (the exact count grows with each release; the invariant is "all pass") • **18 supported source formats** for `bsa materials` staging (rec #3 close): pdf/docx/md/txt/xlsx/csv/tsv/json/graphql/pptx/html/htm/eml/msg/png/jpg/jpeg/tiff/tif • evidence-bound claim layer (INV-01) • closed `ClaimType` enum (INV-07) • tier-aware weighted coverage (KPI-001) • two-key promotion • no-new-claims gate • Phase-3 story-claim provenance (INV-08) + NFR measurability (INV-09) + test-scenario provenance (INV-10).
